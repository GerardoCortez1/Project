 ...
 * Created on February 12, 2023, 10:00 PM
 * Purpose:  Create a game with funcitons and Arrays, including
...
int main(int argc, char** argv) {
    ...
    int btnsnum[4] = {4, 3, 1, 2}; // Array for USER position - using numbers.
    binarySearch(btnsnum, 4, 1);
    bool move = false;
    ...
    //Welcome Part
    cout << "Welcome to Marked Missiles" << endl;
    ...
    while (startGm) {
        if(move){
            selectionSort(enemies, 2, false);
            move = false;
        } else {
            selectionSort(enemies, 2, true);
            move = true;
        }
        // Display gameboard
        cout << "**********" << endl; // 1st Row
...
void display_instructions() {
    cout << "*************************INSTRUCTIONS*************************\n"
            ...
            "or the number of where you want to shoot, then press ENTER and \n"
...
// Function to Display Buttons INT
void display_buttons(int btns[4]) {
    cout << "*";
    for (int i = 0; i < 4; i++) {
        cout << btns[i] << " ";
    };
...
void selectionSort(vector<int> &array, int size, bool lower){
